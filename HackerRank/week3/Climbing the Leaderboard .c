...
int* climbingLeaderboard(int ranked_count, int* ranked, int player_count, int* player, int* result_count) {
    // Remove duplicates from ranked and prepare ranks
    ...
    *result_count = player_count;
    ...
    // Start from the lowest score (end of unique scores)
    int index = unique_count - 1;
    for (int i = 0; i < player_count; i++) {
        while (index >= 0 && player[i] >= scores[index]) {
            index--;
        }
        results[i] = index + 2; // +2 because index+1 is current rank, and we moved one step back
    }
