...
long roadsAndLibraries(int n, int c_lib, int c_road, int cities_rows, int cities_columns, int** cities) {
    ...
    for (int i = 0; i < cities_rows; i++) {
        adj_sizes[cities[i][0]-1]++;
        adj_sizes[cities[i][1]-1]++;
    }

    int** adj = malloc(n * sizeof(int*));
    for (int i = 0; i < n; i++) adj[i] = malloc(adj_sizes[i] * sizeof(int));
    int* idx = calloc(n, sizeof(int));
    for (int i = 0; i < cities_rows; i++) {
        int u = cities[i][0]-1, v = cities[i][1]-1;
        adj[u][idx[u]++] = v;
        adj[v][idx[v]++] = u;
    }
    free(idx);
    ...
    for (int i = 0; i < n; i++) {
        if (!visited[i]) {
            int count = 0;
            dfs(i, &count);
            cost += c_lib + (long)(count - 1) * c_road;
        }
    }
